...
async def create_order(create_order_request: CreateOrderRequest, db: Session = Depends(get_db)) -> None:
    ...
    for item in create_order_request.items:
        ...
        if menu_item is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Item with ID {create_order_request.item_id} not found.")
        order_item: OrderItem = OrderItem(menu_item_id=item.menu_item_id, total_item_quantity=item.total_item_quantity)
...
@app.patch("/orders/{order_id}", status_code=status.HTTP_204_NO_CONTENT)
async def update_order(id: int, updated_fields: UpdateOrderRequest, db: Session = Depends(get_db)) -> None:
...
async def get_customer_orders(customer_id: int, db: Session = Depends(get_db)) -> GetCustomersOrderRequest:
    statement= (
        select(Customer.id, Customer.name, func.coalesce(func.sum(OrderItem.quantity), 0)).select_from(Customer).outerjoin(Order, Order.customer_id == Customer.id).outerjoin(OrderItem, OrderItem.order_id).where(Customer.id == customer_id).group_by(Customer.id, Customer.name)
    )
